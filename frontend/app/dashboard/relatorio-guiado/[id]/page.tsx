...
export default function RelatorioGuiado() {
  ...
  useEffect(() => {
    const buscar = async () => {
      try {
        const docRef = doc(db, "relatorios_guiados", relatorioId);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans flex flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
              ...
              <span className="text-purple-600">Modo Guiado</span>
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="w-full max-w-5xl mx-auto px-4 md:px-8 py-8 flex flex-col gap-10">
        ...
        <section>
          ...
          <div className="bg-white border-2 border-black rounded-[2rem] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] p-5 md:p-7 flex flex-col justify-between gap-4">
            <div className="flex items-center gap-2 pb-3 border-b-2 border-slate-100">
              ...
              <h2 className="text-xs font-black uppercase text-purple-600 tracking-widest">Análise da Sessão</h2>
            </div>
        ...
        {/* ── SEÇÃO 2: AVALIAÇÃO POR PERGUNTA ── */}
        <section className="space-y-6">
          <div className="flex items-center gap-3 border-b-2 border-black pb-3">
            <Activity size={24} className="text-purple-600" />
            <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-black">Avaliação por Pergunta</h2>
          </div>
            ...
            {avaliacoes.map((av: any, i: number) => {
              ...
              return (
                <motion.div key={av.id ?? i}
                  ...
                  <div className="flex flex-wrap items-center gap-3 px-5 py-3 bg-slate-50 border-b-2 border-slate-100">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                      Fase {av.fase} · {FASE_LABEL[av.fase] ?? "—"}
                    </span>
                    <span className="font-black text-sm text-black flex-1">{av.assunto}</span>
                    <span className={`flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest border px-2.5 py-1 rounded-full ${corClass}`}>
                      {cfg.icon} {cfg.label}
                    </span>
                  </div>
        ...
        {/* ── SEÇÃO 6: EXTRATO (respostas brutas) ── */}
          ...
          <div className="flex flex-col gap-3">
            {avaliacoes.map((av: any, i: number) => (
              <div key={i} className="bg-white border border-slate-200 rounded-xl p-4">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{av.assunto}</p>
