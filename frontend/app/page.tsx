...
const NEURAL_LINES = [
  { x2: 34, y2: 40 }, { x2: 60, y2: 39 },
  { x2: 33, y2: 49 }, { x2: 61, y2: 49 },
  { x2: 34, y2: 58 }, { x2: 61, y2: 58 },
];
...
// ─── Canvas component ──────────────────────────────────────────
function NucleusCanvas({ displaySize = SZ }: { displaySize?: number }) {
  ...
  const draw = useCallback(() => {
    ...
    ctx.beginPath(); ctx.arc(CX, CY, RINGS[0], 0, Math.PI * 2);
    ...
    ctx.beginPath(); ctx.arc(CX, CY, 52, 0, Math.PI * 2); ctx.fillStyle = core; ctx.fill();

    const spec = ctx.createRadialGradient(CX - 14, CY - 16, 0, CX - 14, CY - 16, 22);
    spec.addColorStop(0, "rgba(255,255,255,0.55)");
    spec.addColorStop(1, "rgba(255,255,255,0)");
    ctx.beginPath(); ctx.arc(CX - 14, CY - 16, 22, 0, Math.PI * 2); ctx.fillStyle = spec; ctx.fill();
  ...
  return (
    <motion.div
      ...
      className={`absolute flex items-center gap-2 px-3 py-1.5 rounded-full border text-[11px] font-black uppercase tracking-wider whitespace-nowrap backdrop-blur-sm pointer-events-none select-none ${
        ok ? "bg-emerald-50/90 border-emerald-200 text-emerald-700"
...
function FloatingCard({ children, delay, amp = 10, style }: {
  children: React.ReactNode; delay: number; amp?: number; style?: React.CSSProperties;
}) {
  return (
    <motion.div
...
function CardLivre() {
  ...
  return (
    <div className="p-3 space-y-2.5">
      ...
      <p className="text-[11px] font-bold text-slate-600 leading-tight">Explique sem roteiro.<br />O sistema analisa tudo.</p>
      <div className="flex items-end gap-[3px] h-8">
        {barHeights.map((h, i) => (
          <motion.div key={i} className="flex-1 bg-blue-400 rounded-sm"
      ...
      <div className="flex items-center gap-1.5">
        <motion.span className="w-1.5 h-1.5 rounded-full bg-blue-500 flex-shrink-0"
  ...
  return (
    <div className="p-3 space-y-2.5">
      ...
      <div className="bg-slate-50 rounded-xl p-2.5 border border-slate-100">
        ...
        <p className="text-[11px] font-bold text-slate-700 leading-tight">"Qual a diferença entre revogação e anulação?"</p>
      </div>
      <div className="flex items-center justify-between">
        <div className="flex gap-1">
          {[1,2,3,4,5].map((n) => (
            <div key={n} className={`w-4 h-1 rounded-full ${n <= 2 ? "bg-emerald-400" : "bg-slate-200"}`} />
          ))}
        </div>
...
function CardSimulado() {
  return (
    <div className="p-3 space-y-2.5">
      ...
      <p className="text-[11px] font-bold text-slate-600 leading-tight">Resolução de problemas e foco em questões discursivas como em prova real.</p>
      <div className="bg-violet-50 rounded-xl p-2 border border-violet-100 flex items-center gap-2">
        <motion.div className="w-2 h-2 rounded-full bg-violet-500 flex-shrink-0"
      ...
      <div className="grid grid-cols-2 gap-1">
        {[{ label: "Discursiva", color: "text-violet-600", bg: "bg-violet-50" }, { label: "Problema", color: "text-slate-500", bg: "bg-slate-50" }].map(({ label, color, bg }) => (
      ...
      >
        {Array.from({ length: 2 }).flatMap((_, copy) =>
          ["CONCURSO", "EXAME DA OAB", "ENEM", "CONCURSO", "EXAME DA OAB", "ENEM"].map((item, i) => (
            <span key={`${copy}-${i}`} className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500 flex-shrink-0 flex items-center px-4 gap-4">
...
export default function LandingPage() {
  ...
  const [canvasSize, setCanvasSize]   = useState(SZ);
  ...
  useEffect(() => {
    const update = () => setCanvasSize(window.innerWidth < 768 ? Math.min(window.innerWidth - 48, SZ) : SZ);
    update();
  ...
  return (
    <div className="min-h-[100dvh] relative flex flex-col" style={{ background: "#F4F6FB" }}>
      ...
      {/* Particles */}
      {PARTICLES.map((p) => (
        <motion.div key={p.id} className="absolute rounded-full bg-blue-400 pointer-events-none"
      ...
      <nav className="relative z-20 flex items-center justify-between px-4 md:px-8 h-16 md:h-20 flex-shrink-0 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            ...
            className="text-xs font-black uppercase tracking-widest text-slate-500 hover:text-slate-900 transition-colors px-3 py-2 min-h-[40px]">
            Entrar
          </motion.button>
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={() => router.push("/login")}
            className="text-xs font-black uppercase tracking-widest bg-black text-white px-4 py-2.5 rounded-xl hover:bg-blue-600 transition-colors shadow-sm min-h-[40px]">
            Começar →
          </motion.button>
        </div>
      </nav>
      ...
      {/* ══════════════════════════════════════════════
          MOBILE layout  (< md)
      ══════════════════════════════════════════════ */}
      <div className="md:hidden flex flex-col items-center flex-1 px-5 py-6 gap-6 z-10">
        ...
        <motion.h1
          ...
          className="font-black text-black leading-tight tracking-tight text-center text-[1.85rem]"
        >
          Pare de reler.
          <br />
        ...
        >
          {[0, 1, 2].map((i) => (
            <motion.div key={i}
              ...
              style={{ top: "50%", left: "50%", width: canvasSize, height: canvasSize, marginTop: -canvasSize / 2, marginLeft: -canvasSize / 2 }}
              animate={{ scale: [1, 3.2], opacity: [0.35, 0] }}
              transition={{ duration: 3.5, delay: i * 1.15, repeat: Infinity, ease: "easeOut" }}
            />
          ))}
          <NucleusCanvas displaySize={canvasSize} />
        </motion.div>

        {/* Description + CTA + Ticker */}
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <p className="text-center text-slate-500 font-semibold text-sm leading-snug">
      ...
      ══════════════════════════════════════════════ */}
      <div className="hidden md:block relative flex-1 z-10" style={{ height: "calc(100dvh - 80px)" }}>
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
          {NEURAL_LINES.map((line, i) => (
            <motion.line key={i} x1="50%" y1="50%" x2={`${line.x2}%`} y2={`${line.y2}%`}
              stroke="rgba(37,99,235,0.12)" strokeWidth="1" strokeDasharray="4 6"
        ...
        >
          <NucleusCanvas displaySize={SZ} />
        </motion.div>

        {/* Side cards (xl) */}
        <FloatingCard delay={1.8} amp={8} style={{ left: "2%", top: "15%" }}><CardRelatorio /></FloatingCard>
        <FloatingCard delay={2.2} amp={12} style={{ left: "2%", top: "55%" }}><CardLivre /></FloatingCard>
        <FloatingCard delay={2.0} amp={10} style={{ right: "2%", top: "18%" }}><CardGuiado /></FloatingCard>
        <FloatingCard delay={2.4} amp={9}  style={{ right: "2%", top: "58%" }}><CardSimulado /></FloatingCard>

        {/* Subheadline + CTA */}
        <motion.div
          initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.4, ease: "easeOut" }}
          className="absolute bottom-[5%] left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 md:gap-5 w-full max-w-xl px-6"
        >
          <p className="text-center text-slate-500 font-semibold leading-snug"
